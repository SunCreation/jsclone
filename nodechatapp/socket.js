/* //socker.io 패키지 참조
const SockerIO = require("socker.io");

//socker.js 모듈 기능 정의
module.exports =(server)=>{

	//express서버와 socker.io객체 연결
	const io = SockerIO(server, {path:"/socker.io"});


	//socker.io connection 이벤트 정의 추가
	//웹브라우저와 웹서버(express)간 소켓통신 연결이 완료되면 발생하는 connection 이벤트 기능 정의
	//웹브라우저와 웹서버가 연결된상태 발생하는 모든 기능을 콜백함수 안에서 구현한다.
	io.on("connection",(socket)=>{

		//브라우저에서 호출되는 서버측 기능1
		//기능 이름은 임의로 정의하며 해당 기능이 호출될 때 콜백함수가 실행된다.
		//콜백함수가 호출될 때 브라우저에서 전달한 값을 파라메터로 수신받는다.
		socker.on("broadcast",function(msg){

			//브라우저에서 전송된 msg데이터를 현재 연결된 모든 사용자 브라우저에게 전송한다.
			//receiveAll은 브라우저에서 서버로부터 전송된 메세지를 받을 기능명이다.
			io.emit("receiveAll",msg);

			//socker.broadcast.emit("receive",msg);
		});
